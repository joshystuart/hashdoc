...
describe('branded OG card (built output)', () => {
  it.runIf(built)(
    'built index.html has the static Open Graph meta tags',
    () => {
      ...
      expect(html).toMatch(
        /<meta[^>]+property=["']og:title["'][^>]+content=["']HashDoc["']/i,
      );
      expect(html).toMatch(
  ...
  it.runIf(built)(
    'built index.html has the static Twitter card meta tags',
    () => {
      const html = readFileSync(indexHtml, 'utf8').replace(
        /<!--[\s\S]*?-->/g,
        '',
      ...
      expect(html).toMatch(
        /<meta[^>]+name=["']twitter:title["'][^>]+content=["']HashDoc["']/i,
      );
  ...
  it.runIf(built)(
    'the OG image is self-hosted: present in dist at the referenced path',
    () => {
      const png = join(distDir, 'og-card.png');
      expect(
        existsSync(png),
        'dist/og-card.png must exist (copied from public/)',
      ).toBe(true);
  ...
  it.runIf(built)(
    'the OG image path is root-relative, not an external origin',
    () => {
  ...
  it('the reviewable SVG source is kept in public/', () => {
    ...
    expect(existsSync(svg), 'public/og-card.svg source must be committed').toBe(
      true,
    );
    const text = readFileSync(svg, 'utf8');
    const markup = text.replace(/<!--[\s\S]*?-->/g, '');
    const scannable = markup.replace(
      /xmlns=["']http:\/\/www\.w3\.org\/[^"']*["']/g,
      '',
    );
