...
- **Inherently-exposed metadata stays exposed:** the approximate Document size (Link length) and the fact that a Link is secure (the `2` tag) are observable.
...
- **v2 is async** (Web Crypto is Promise-based) while v1 stays synchronous. Callers branch up front with the cheap synchronous `isSecure` check and only await decryption when a password is supplied.
...
- **Unsecure Links make no security claims.** Only secure surfaces may use security language; v1 bearer Links remain honestly plain.
