...
The reader experience for opening a secure (tag `2`) Link in the Viewer,
...
type ViewerState =
  ...
  | { kind: 'editor' }
  | { kind: 'locked'; payload: string }   // new: secure Link, awaiting password
  | { kind: 'error'; errorKind: DecodeErrorKind };
```

`resolveView` detects a secure Payload via `isSecure` and returns `locked`
...
awaits `decodeSecure(payload, password)`:
...
distinction: **"Copy Link" must re-emit the original secure Link** (the `2…`
...
- [x] A secure Link routes `resolveView` to `kind: 'locked'` synchronously
...
- [x] A truncated/corrupt secure Link shows the existing corrupt-Link error
      view.
- [x] After unlock, "Copy Link" copies a still-secure Link (its Payload starts
...
- [x] Plain (tag `1`) Links open exactly as before — no prompt, no
      behavior change.
